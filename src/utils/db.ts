 ...
 */
export const createTrack = async (track: Omit<Track, 'id' | 'createdAt' | 'updatedAt'>): Promise<Track> => {
	...
	await fs.writeFile(path.join(TRACKS_DIR, `${id}.json`), JSON.stringify(newTrack, null, 2))
 ...
 */
export const updateTrack = async (id: string, updates: Partial<Track>): Promise<Track | null> => {
	try {
		...
		await fs.writeFile(path.join(TRACKS_DIR, `${id}.json`), JSON.stringify(updatedTrack, null, 2))
 ...
 */
export const saveAudioFile = async (id: string, fileName: string, buffer: Buffer): Promise<string> => {
